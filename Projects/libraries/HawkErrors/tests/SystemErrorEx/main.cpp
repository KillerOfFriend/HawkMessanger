...
TEST(SystemErrorEX, Create)
{
    ...
    const std::uint32_t Begin = errors::C_CATEGORY_SYSTEMERROREX_START;
    const std::uint32_t End = static_cast<std::uint32_t>(errors::eSystemErrorEx::seCount);

    for (std::uint32_t ErrorCode = Begin; ErrorCode < End; ++ErrorCode)
    {   // Создадим все входящие в категорию ошибки
        Error = make_error_code(static_cast<errors::eSystemErrorEx>(ErrorCode));
